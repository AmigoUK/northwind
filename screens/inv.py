...
class INVDetailModal(ModalScreen):
    ...
    def on_pdf(self) -> None:
        ...
        suggested = f"northwind_inv_{self.inv_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        def after(path):
            if path:
                try:
                    import pdf_export
                    pdf_export.export_inv(self.inv_id, save_path=path)
                    self.notify(f"PDF saved → {path}", severity="information")
                except Exception as e:
                    self.notify(f"PDF error: {e}", severity="error")
        self.app.push_screen(FileSelectModal(title="Save Invoice PDF", mode="save",
